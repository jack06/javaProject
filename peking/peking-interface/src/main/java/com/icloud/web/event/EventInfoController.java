...
public class EventInfoController extends BaseController {
	...
	public String getEventsListByMonth(HttpServletRequest request) throws Exception {
		...
		for (int i = 0; null != list && i < list.size(); i++) {
			...
			ev.put("latitude", event.getLatitude());
			ev.put("longitude",event.getLongitude());
			ev.put("eventType", event.getEventType().getTypeName());
	 ...
	 */
	@SuppressWarnings("deprecation")
	@RequestMapping("/getEventsListByFriend")
	public String getEventsListByFriend(HttpServletRequest request) throws Exception {
		...
		int pageNo = StringUtils.isNotBlank(offset) ? Integer.parseInt(offset) : 1;
		int pageSize = StringUtils.isNotBlank(size) ? Integer.parseInt(size) : 20;
		PageInfo<EventInfo> page = eventInfoService.findFriendEvents(pageNo, pageSize, event);
		List<EventInfo> list = page.getList();
		JSONObject resultData = new JSONObject();
		JSONArray eventArray = new JSONArray();
		JSONObject ev = null;
	...
	public String getAnnouncementList(HttpServletRequest request) throws Exception{
		JSONObject parameterObj = super.readToJSONObect(request);
		JSONObject resultObj = new JSONObject();
		User user = userService.getUserFromSession(parameterObj.getString("sid"));
		if (null == user) {
		...
		event.setPublisherType("system");
		
		int pageNo = StringUtils.isNotBlank(offset) ? Integer.parseInt(offset) : 1;
		int pageSize = StringUtils.isNotBlank(size) ? Integer.parseInt(size) : 20;
		PageInfo<EventInfo> page = eventInfoService.findForAnnouncementList(pageNo, pageSize, event);
		List<EventInfo> list = page.getList();
		JSONObject resultData = new JSONObject();
		JSONArray eventArray = new JSONArray();
		JSONObject ev = null;
		...
		resultData.put("offset", page.getPageNum());
		resultData.put("size", page.getPageSize());
		resultObj.put("resultData", resultData);
		resultObj.put("errCode", "0000");
		resultObj.put("resultMsg", "列表获取成功");
		return pakageJsonp(resultObj);
	...
	public String getEventBase(HttpServletRequest request) throws Exception {
        ...
        resultData.put("eventType",event.getEventType().getTypeName());
