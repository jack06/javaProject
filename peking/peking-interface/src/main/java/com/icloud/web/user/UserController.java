...
//	private UserRoleService userRoleService;
	...
	public String addFriend(HttpServletRequest request){
		...
		if(!parameterObj.containsKey("friendId")){
			resultObj.put("errCode", "1000");
			resultObj.put("resultMsg", "参数缺失friendId");
			return pakageJsonp(resultObj);
		}
		...
		String friendId = parameterObj.getString("friendId");
		UserAssociated userAssociated = new UserAssociated();
		userAssociated.setMasterUserId(friendId);
		userAssociated.setSlaveUserId(user.getId());
		int count = userAssociatedService.checkIsFriend(userAssociated);
		if(0>=count){
			userAssociatedService.save(userAssociated);
		}
		resultObj.put("errCode", "0000");// 用户不存在
		resultObj.put("resultMsg", "添加好友成功");
		return pakageJsonp(resultObj);
	...
	public String deleteFriend(HttpServletRequest request){
		...
		userAssociated.setMasterUserId(friendId);
		userAssociated.setSlaveUserId(user.getId());
		userAssociatedService.deleteByUser(userAssociated);
		
		resultObj.put("errCode", "0000");// 用户不存在
		resultObj.put("resultMsg", "删除好友成功");
		return pakageJsonp(resultObj);
